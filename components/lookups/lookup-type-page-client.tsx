...
import {
  createLookupResource,
  deleteLookupResource,
  getLookupResource,
  listLookupResource,
  updateLookupResource,
  type LookupResourcePath,
} from "@/lib/lookup-resource-api";
import type { LookupListItem } from "@/lib/lookups-api";
...
export type LookupTypePageConfig = {
...
export function LookupTypePageClient({ resource, title, description, singularLabel }: LookupTypePageConfig) {
  ...
  const load = React.useCallback(async () => {
    setLoading(true);
    const result = await listLookupResource(resource, { pageNumber: 1, pageSize: 200 });
    setLoading(false);
  ...
  const openEdit = async (row: LookupListItem) => {
    ...
    setSheetOpen(true);
    setSaving(true);
    const result = await getLookupResource(resource, row.id);
    setSaving(false);
    if (!result.success || !result.data) {
  ...
  const onDelete = async (row: LookupListItem) => {
    ...
    const result = await deleteLookupResource(resource, row.id);
    if (!result.success) {
  ...
  const onSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    if (editingId === null) {
      const result = await createLookupResource(resource, { name: name.trim(), isActive: true });
      setSaving(false);
      if (!result.success) {
        toast.error(apiErrorMessage(result, "Create failed"));
        return;
      }
      toast.success(result.message ?? `${singularLabel} created`);
    } else {
      if (!rowVersion) {
        setSaving(false);
      ...
      const result = await updateLookupResource(resource, editingId, {
        name: name.trim(),
        isActive: true,
        rowVersion,
      });
  ...
  return (
    <div className="flex flex-col gap-4 px-4 lg:px-6">
            ...
            {loading ? (
              ...
              <TableRow>
                <TableCell colSpan={3} className="text-center text-muted-foreground">
                  No items yet — add one for application forms.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent>
          <SheetHeader>
            <SheetTitle>{editingId === null ? `Add ${singularLabel.toLowerCase()}` : `Edit ${singularLabel.toLowerCase()}`}</SheetTitle>
          </SheetHeader>
